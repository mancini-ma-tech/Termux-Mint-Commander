...
def esegui_azione(comando):
    ...
    if comando == "NOTIFICA":
        subprocess.run(
            ["notify-send", "📱 Telecomando S25", "Messaggio recapitato!"])
    ...
    # --- ECCO IL NUOVO TASTO DEL TELECOMANDO ---
    elif comando == "BLOCCO":
        # Questo comando blocca immediatamente la sessione di Linux Mint Cinnamon
        subprocess.run(["cinnamon-screensaver-command", "--lock"])
        print("🔒 Schermo bloccato. Fortezza attiva.\n")

    else:
        print("❓ Comando sconosciuto. Non faccio nulla.\n")
